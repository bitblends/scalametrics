...
case class ProjectRollup(
  ...
  /**
    * Converts the properties of the current instance into a map where the keys are property names and the values are
    * their string representations.
    *
    * The method handles various types of property values such as:
    *   - `Some(v)`: Converts to the string representation of `v`.
    *   - `None`: Inserts an empty string as the value.
    *   - `Seq[_]`: Joins the elements into a comma-separated string.
    *   - All other types: Converts directly to their string representation.
    *
    * @return
    *   A map where keys are the property names of the instance and values are their string representations.
    */
