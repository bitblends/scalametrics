...
import scala.meta.{Dialect, Init, Mod, Parsed, Pkg, Token, Tokens, Tree, Type, XtensionParseInputLike, XtensionSyntax}

/**
  * The `Util` class provides a collection of utility methods for analyzing and processing Scala source code, including
  * parsing, extracting information from abstract syntax trees (AST), and determining various code properties.
  */
object Util:
  ...
  def extractPackageName(tree: Tree): String = {
    // Find package object name (first occurrence) without mutation
    def findPackageObjectName(t: Tree): Option[String] = t match {
      case Pkg.Object(_, name, _) => Some(name.value)
      case _                      =>
        t.children.iterator
          .map(findPackageObjectName)
          .collectFirst { case Some(n) => n }
    }

    // Collect all package declarations in a pre-order traversal
    def collectPackages(t: Tree): List[String] = t match {
      case pkg: Pkg =>
        val ref = pkg.ref.toString()
        val current = if (ref.nonEmpty) List(ref) else Nil
        current ++ pkg.children.flatMap(collectPackages)
      case _ =>
        t.children.flatMap(collectPackages)
    }

    val packageParts = collectPackages(tree)
    val packageObjectName = findPackageObjectName(tree)

    (packageParts, packageObjectName) match {
      case (Nil, _)                        => "<default>"
      case (head :: _, Some(objName))      => s"$head.$objName"
      case (single :: Nil, None)           => single
      case (parts, None) if parts.nonEmpty => parts.mkString(".")
      case _                               => "<default>"
    }
  }
  ...
  def isDeprecated(mods: List[Mod], defn: Tree, allTokens: Tokens): Boolean =
    Util.hasDeprecatedAnnotation(mods) || Util.hasDeprecatedDoc(defn, allTokens)
  ...
  def lastTypeName(t: Type): String = t match {
    ...
    case Type.Apply(tpe, _)            => Util.lastTypeName(tpe)
    case Type.Annotate(tpe, _)         => Util.lastTypeName(tpe)
  ...
  private def isPublicMod(mods: List[Mod]): Boolean = {
    // A function is public if it has no access modifier
    !mods.exists {
