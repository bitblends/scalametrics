...
case class PackageRollup(
  ...
  /**
    * Generates a formatted string representation of the statistical metrics contained within the `PackageRollup`
    * instance. The output includes details about functions, inline usage, implicits, given instances, pattern matching
    * metrics, and branch density metrics, presented in a structured format.
    *
    * @return
    *   A formatted string summarizing the metrics of the `PackageRollup`.
    */
  def prettify: String = {
    f"""Package: $name
  ...
  /**
    * Converts the statistical metrics of the `PackageRollup` instance into a map representation. Each entry in the map
    * corresponds to a metric name and its associated value.
    *
    * @return
    *   A map where keys are metric names as strings (e.g., "name", "totalFunctions") and values are their corresponding
    *   string representations. If a metric has a value of `None`, an empty string is used.
    */
