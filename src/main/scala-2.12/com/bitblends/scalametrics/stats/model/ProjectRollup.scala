...
case class ProjectRollup(
  ...
  /**
    * Generates a formatted string representation of various statistical metrics and aggregated information related to a
    * project, including overall statistics, function metrics, symbol metrics, documentation and deprecation data,
    * return type explicitness, inline and implicit usage, pattern matching metrics, branch density metrics, and
    * complexity metrics.
    *
    * @return
    *   A detailed, human-readable string outlining the project's statistics and metrics.
    */
  def prettify: String = {
  ...
  /**
    * Converts the statistical data of the current instance into a map representation. Each field name is used as a key,
    * and its corresponding value is included as the map value. If a field value is optional, its `Some` value is
    * converted to a string, or an empty string is used if the value is `None`. Sequences are concatenated into a
    * comma-separated string.
    *
    * @return
    *   A map where the keys are field names and the values are their corresponding string representations.
    */
  def toMap: Map[String, String] = {
    val fieldNames = List(
      ...
      // Package-level summary,
      "totalPackages",
      "packagesWithHighComplexity",
      "packagesWithLowDocumentation"
    )
    fieldNames
