...
case class ProjectStatsHeader(
  ...
  /**
    * Converts the properties of the `ProjectStatsHeader` instance into a map representation. Each field of the instance
    * is paired with its corresponding value as a string. For optional or collection fields, appropriate string
    * representations are provided:
    *   - `Some` values are converted to their inner value as a string.
    *   - `None` is represented as an empty string.
    *   - Sequences are converted to a comma-separated string of their elements.
    *
    * @return
    *   A map where the keys represent the names of the fields in `ProjectStatsHeader`, and the values represent their
