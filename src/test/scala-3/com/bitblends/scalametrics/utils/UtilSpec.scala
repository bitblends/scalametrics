...
import com.bitblends.scalametrics.metrics.TraverserCompat
...
class UtilSpec extends AnyFlatSpec with Matchers {
  ...
  // Helper to collect nodes from a tree
  private def collectDefs(tree: Tree): List[Defn.Def] = {
    val buffer = scala.collection.mutable.ListBuffer.empty[Defn.Def]
    object Collector extends TraverserCompat {
      override def apply(tree: Tree): Unit = tree match {
        case d: Defn.Def => buffer += d; traverseChildren(tree)
  ...
  private def collectVals(tree: Tree): List[Defn.Val] = {
    val buffer = scala.collection.mutable.ListBuffer.empty[Defn.Val]
    object Collector extends TraverserCompat {
      override def apply(tree: Tree): Unit = tree match {
        case v: Defn.Val => buffer += v; traverseChildren(tree)
        case _           => traverseChildren(tree)
      }
    }
    Collector(tree)
  ...
  it should "count LOC for code with trailing newline" in {
    val code = "line1\nline2\nline3\n"
    val loc = Util.countLOC(code)
    loc shouldBe 3 // Trailing newline
  }
  ...
  it should "detect Scaladoc comment before definition" in {
    ...
    val defn = collectDefs(tree).head
  ...
  it should "detect @deprecated tag in Scaladoc" in {
    ...
    val defn = collectDefs(source).head
  ...
  it should "handle file with implicit conversions" in {
    ...
    try {
      ...
      try {
        ...
        parsed shouldBe defined
        val defs = collectDefs(parsed.get)
        val vals = collectVals(parsed.get)
